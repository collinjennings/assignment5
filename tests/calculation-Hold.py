...
def test_str_representation():
    ...
    # Assuming you have a Calculation class with these attributes
    # You'll need to adjust based on your actual class implementation
    
    # Test basic string formatting
    calc = Calculation(operation="add", operand1=5, operand2=3, result=8)
    assert str(calc) == "add(5, 3) = 8"
    
    # Test with different operation
    calc = Calculation(operation="multiply", operand1=4, operand2=7, result=28)
    assert str(calc) == "multiply(4, 7) = 28"
    
    # Test with negative numbers
    calc = Calculation(operation="subtract", operand1=10, operand2=-5, result=15)
    assert str(calc) == "subtract(10, -5) = 15"
    
    # Test with floating point numbers
    calc = Calculation(operation="divide", operand1=10.5, operand2=2.0, result=5.25)
    assert str(calc) == "divide(10.5, 2.0) = 5.25"
    
    # Test with zero
    calc = Calculation(operation="add", operand1=0, operand2=0, result=0)
    assert str(calc) == "add(0, 0) = 0"